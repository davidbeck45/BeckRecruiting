...
### 0.1 What the Family Night deck adds (product requirements)

Coach Beck's "Protect The Family. Place The Player." deck turns three of his promises into hard platform features:

1. **The written level projection.** The Sophomore Recruiting Kickoff promises every player a formal, written projection (P4 / G5 / FCS / D2 / D3 / NAIA) plus a film plan, target list, and academic runway. The platform stores projections as first-class records — who projected, when, at what level — because promise #4 depends on it.
2. **Certified measurables.** "No inflated stopwatch times — colleges know our page is real" is the trust moat. Every measurable carries a verification record: value, method (laser, coach-certified), who signed off, when. Outreach emails only cite verified numbers; the AI composer is already instructed to never invent stats, and verification is what makes the cited ones credible.
3. **The Receipts.** "[XX]% signed at or above projected level — we publish the number" is the single most powerful marketing asset in the deck, and it is *only computable if the data is captured from the first athlete onward*: projection at sophomore spring, signing outcome at the end. The platform computes this stat automatically (`GET /receipts`).
4. **Tiered targeting, not mass email.** The deck explicitly positions against mass email: "15–20 realistic schools, tiered — worked coach-to-coach." That sets the platform's default posture: the target list is small and realistic; Coach Beck's calls go to the schools where his relationships are live; the platform's personalized email is the *written first-touch and follow-up layer* for the same tiered list — and its engagement telemetry (who watched film) tells him which coaches to call first. Volume distribution beyond the tiered list is an explicit family opt-in, not the default.
5. **The follow-up loop.** "We call every college that signs our players, twice a year" — signings are stored with the school, so the platform can generate the twice-yearly call list automatically.
